...
public class OBJ : MonoBehaviour
{
    ...
    private IEnumerator _Load(string path)
    {
        ...
        UnityWebRequest geomRequest = UnityWebRequest.Get(path);
        yield return geomRequest.SendWebRequest();
        if (geomRequest.result != UnityWebRequest.Result.Success)
        {
    ...
    private static Shader FindShaderOrFallback(string primary, params string[] fallbacks)
    {
        ...
        {
            Shader candidate = Shader.Find(primary);
        ...
        if (fallbacks != null)
        {
            foreach (string fallback in fallbacks)
            {
                ...
                Shader candidate = Shader.Find(fallback);
                if (candidate != null) return candidate;
            }
        }
    ...
    private static void ConfigureStandardMaterial(Material material, bool transparent, float alpha, float roughness,
        Color diffuse, Texture2D diffuseTexture, Color specular)
    {
        material.color = new Color(diffuse.r, diffuse.g, diffuse.b, transparent ? alpha : 1f);
        ...
        {
            metallic = Mathf.Clamp01(specular.maxColorComponent);
        }
    ...
    private Material GetMaterial(MaterialData md)
    {
        ...
        bool isSpecular = md.illumType is 2 or 3 or 5;
        bool isTransparent = md.illumType is 4 or 6 or 7 or 9;

        string shaderName = isTransparent ? "Poly/PbrBlendDoubleSided" : "Poly/PbrOpaqueDoubleSided";
        Shader shader = FindShaderOrFallback(shaderName, "Standard");
        m = new Material(shader);

        if (shader != null && shader.name == "Standard")
        {
            ConfigureStandardMaterial(m, isTransparent, md.alpha, roughness, md.diffuse, md.diffuseTex, md.specular);
        }
        else
        {
            if (isSpecular)
            ...
            var ks = md.specular;
            if (ks.r != ks.g || ks.g != ks.b)
            {
                if (ks.maxColorComponent > 0.01f)
                {
    ...
    private void Build()
    {
        ...
        if (hasMaterials)
        {
            foreach (MaterialData md in materialData)
            {
                ...
                Material mat = GetMaterial(md);
                materials.Add(md.name, mat);
            }
        }
        else
        {
            Shader fallback = FindShaderOrFallback("Standard");
